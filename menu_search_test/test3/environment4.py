...
class Environment(gym.Env, ABC, Interface):
    ...
    def __init__(self, envconfig):
        ...
        self.n_targets = envconfig['n_targets']
        ...
        obs_space_dict = {
            ...
            'menu': spaces.Box(low=np.array([0.0, 0.0] * self.n_slots), 
                               high=np.array([1.0, 1.0] * self.n_slots), 
                               dtype=np.float64), 
    ...
    def get_reward(self):
        ...
        for target in targets:
            ...
            if self.state['selected_slots'][target] == 0:
                reward *= 4*calc_distance(self.state['cursor_position'], target_center,'l2')
    ...
    def reset(self):
        ...
        idx = np.random.randint(low=0, high=self.n_slots, size=self.n_targets)
        while len(set(idx)) != self.n_targets: # Make sure we have num_targets different targets
            idx = np.random.randint(low=0, high=self.n_slots, size=self.n_targets)
        slot[idx] =  1
        ...
        self.add['deterministic_action'] = np.zeros(2)
