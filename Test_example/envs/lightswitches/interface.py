...
class Interface:
    ...
    def generate_random_ui(self):
        cells_width = int(self.screen_width / 10)
        cells_height = int(self.screen_width / 10)
        size = 0.15
        margin = 0.01
        _element = {
            "x": None,
        ...
        for target, item in zip(range(self.n_targets), range(self.n_items)):
    ...
    def generate_static_ui(self):
        ...
        assert self.n_targets <= max_per_direction ** 2
        col = 0
        row = 0
        for target, item in zip(range(self.n_targets), range(self.n_items)):
            element = _element.copy()
